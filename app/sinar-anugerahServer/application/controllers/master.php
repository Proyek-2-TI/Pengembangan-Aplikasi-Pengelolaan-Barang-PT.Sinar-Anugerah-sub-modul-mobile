<?php
class Master extends CI_Controller{
    function __construct(){
        parent::__construct();
        if($this->session->userdata('login_status') != TRUE ){
            $this->session->set_flashdata('notif','USERNAME ATAU PASSWORD ANDA SALAH ');
            redirect('');
        };
        $this->load->helper('currency_format_helper');
    }

    function barang_data(){
        $data=array(
            'title'=>'Data Admin',
            'kd_barang'=>$this->model_app->getKodeBarang(),
            'id_jenis_barang'=>$this->model_app->getKodeJenisBarang(),
            'data_barang'=>$this->model_app->getAllDataBarang(),
            'data_jenis_barang'=>$this->model_app->getAllData('tbl_jenis_barang'),
            'data_supplier'=>$this->model_app->getAllData('tb_supplier'),
        );
        $this->load->view('element/v_header',$data);
        $this->load->view('pages/admin/v_barang');
        $this->load->view('element/v_footer');
    }

    function barang_data_sales(){
        $data=array(
            'title'=>'Data Admin',
            'kd_barang'=>$this->model_app->getKodeBarang(),            
            'data_barang'=>$this->model_app->getAllData('tbl_barang'),
        );
        $this->load->view('element/v_header',$data);
        $this->load->view('pages/penjualan/v_barang');
        $this->load->view('element/v_footer');
    }

    function pelanggan_data(){
        $data=array(
            'title'=>'Data Admin',
            'kd_pelanggan'=>$this->model_app->getKodePelanggan(),
            'data_pelanggan'=>$this->model_app->getAllData('tbl_pelanggan'),
        );
        $this->load->view('element/v_header',$data);
        $this->load->view('pages/admin/v_pelanggan');
        $this->load->view('element/v_footer');
    }

    function supplier_data(){
        $data=array(
            'title'=>'Data Admin',
            'kd_supplier'=>$this->model_app->getKodeSupplier(),
            'data_supplier'=>$this->model_app->getAllData('tb_supplier'),
        );
        $this->load->view('element/v_header',$data);
        $this->load->view('pages/admin/v_supplier');
        $this->load->view('element/v_footer');
    }

    function  pegawai_data(){
        $data=array(
            'title'=>'Data Admin',
            'kd_pegawai'=>$this->model_app->getKodePegawai(),
            'data_pegawai'=>$this->model_app->getAllData('tbl_pegawai'),
        );
        $this->load->view('element/v_header',$data);
        $this->load->view('pages/admin/v_pegawai');
        $this->load->view('element/v_footer');
    }

    function pegawai_sales(){
        $data=array(
            'title'=>'Data Admin',
            'active_job_pegawai'=>'active',
            'data_penjualan'=>$this->model_app->getAllDataPenjualan(),
            
        );
        $this->load->view('element/v_header',$data);
        $this->load->view('pages/admin/v_job_pegawai');
        $this->load->view('element/v_footer');
    }

    function pergudangan(){
        $data=array(
            'title'=>'Data pergudangan',
            'active_pergudangan'=>'active',
            'data_pemesanan'=>$this->model_app->getAllDataPemesanan(),
            'data_barang_masuk'=>$this->model_app->getAllDataBarangMasuk(),
            'data_barang_return'=>$this->model_app->getAllDataBarangReturn(),            
        );
        $this->load->view('element/v_header',$data);
        $this->load->view('pages/admin/v_job_pergudangan');
        $this->load->view('element/v_footer');
    }


//
//    ===================== INSERT =====================
    function tambah_barang(){
        $data=array(
            'kd_barang'=>$this->input->post('kd_barang'),
            'nm_barang'=>$this->input->post('nm_barang'),
            'kd_supplier'=>$this->input->post('kd_supplier'),
            'stok'=>$this->input->post('stok'),
            'id_jenis_barang'=>$this->input->post('id_jenis_barang'),
            'harga'=>$this->input->post('harga'),
        );
        $this->model_app->insertData('tbl_barang',$data);
        redirect("master/barang_data");
    }
    function tambah_jenis_barang(){
        $data=array(
            'id_jenis_barang'=>$this->input->post('id_jenis_barang'),
            'jenis_barang'=>$this->input->post('jenis_barang'),
        );
        $this->model_app->insertData('tbl_jenis_barang',$data);
        redirect("master/barang_data");
    }
    function tambah_pelanggan(){
        $data=array(
            'kd_pelanggan'=> $this->input->post('kd_pelanggan'),
            'nm_pelanggan'=>$this->input->post('nm_pelanggan'),
            'alamat'=>$this->input->post('alamat'),
            'email'=>$this->input->post('email'),
        );
        $this->model_app->insertData('tbl_pelanggan',$data);
        redirect("master/pelanggan_data");
    }

    function tambah_pegawai(){
        $data=array(
            'kd_pegawai'=> $this->input->post('kd_pegawai'),
            'username'=>$this->input->post('username'),
            'password'=>md5($this->input->post('password')),
            'nama'=> $this->input->post('nama'),
            'level'=>$this->input->post('level'),
        );
        $this->model_app->insertData('tbl_pegawai',$data);
        redirect("master/pegawai_data");
    }
    
    function tambah_supplier(){
        $data=array(
            'kd_supplier'=> $this->input->post('kd_supplier'),
            'nm_supplier'=>$this->input->post('nm_supplier'),
            'alamat'=>$this->input->post('alamat'),
            'email'=>$this->input->post('email'),
        );
        $this->model_app->insertData('tb_supplier',$data);
        redirect("master/supplier_data");
    }


//    ======================== EDIT =======================
    function edit_barang(){
        $id['kd_barang'] = $this->input->post('kd_barang');
        $data=array(
            'nm_barang'=>$this->input->post('nm_barang'),
            'id_jenis_barang'=>$this->input->post('id_jenis_barang'),
            'kd_supplier'=>$this->input->post('kd_supplier'),
            'stok'=>$this->input->post('stok'),
            'harga'=>$this->input->post('harga'),
        );
        $this->model_app->updateData('tbl_barang',$data,$id);
        redirect("master/barang_data");
    }

    function edit_data_barang(){
        $id= $this->uri->segment(3);
        $data=array(
            'title'=>'Data Admin',
            'barang_data'=>'active',
            'data'=>$this->model_app->getBarang($id),
            'data_jenis_barang'=>$this->model_app->getAllData('tbl_jenis_barang'),
            'data_supplier'=>$this->model_app->getAllData('tb_supplier'),
        );
        $this->load->view('element/v_header',$data);
        $this->load->view('pages/admin/v_edit_barang');
        $this->load->view('element/v_footer');
    }

    function edit_pelanggan(){
        $id['kd_pelanggan'] = $this->input->post('kd_pelanggan');
        $data=array(
            'nm_pelanggan'=>$this->input->post('nm_pelanggan'),
            'alamat'=>$this->input->post('alamat'),
            'email'=>$this->input->post('email'),
        );
        $this->model_app->updateData('tbl_pelanggan',$data,$id);
        redirect("master/pelanggan_data");
    }

    function edit_data_pelanggan(){
        $id= $this->uri->segment(3);
        $data=array(
            'title'=>'Edit Data Pelanggan',
            'data'=>$this->model_app->getPelanggan($id),
        );
        $this->load->view('element/v_header',$data);
        $this->load->view('pages/admin/v_edit_pelanggan');
        $this->load->view('element/v_footer');
    }

    function edit_supplier(){
        $id['kd_supplier'] = $this->input->post('kd_supplier');
        $data=array(
            'nm_supplier'=>$this->input->post('nm_supplier'),
            'alamat'=>$this->input->post('alamat'),
            'email'=>$this->input->post('email'),
        );
        $this->model_app->updateData('tb_supplier',$data,$id);
        redirect("master/supplier_data");
    }    

    function edit_data_supplier(){
        $id= $this->uri->segment(3);
        $data=array(
            'title'=>'Edit Data Supplier',
            'data'=>$this->model_app->getSupplier($id),
        );
        $this->load->view('element/v_header',$data);
        $this->load->view('pages/admin/v_edit_supplier');
        $this->load->view('element/v_footer');
    }

    function edit_contact(){
        $id['id'] = 1;
        $data=array(
            'nama'=> $this->input->post('nama'),
            'owner'=>$this->input->post('owner'),
            'alamat'=>$this->input->post('alamat'),
            'telp'=>$this->input->post('telp'),
            'email'=>$this->input->post('email'),
            'website'=>$this->input->post('website'),
            'desc'=>$this->input->post('desc'),
        );
        $this->model_app->updateData('tbl_contact',$data,$id);
        redirect("master");
    }
    function edit_pegawai(){
        $id['kd_pegawai'] = $this->input->post('kd_pegawai');
        $data=array(
            'username'=>$this->input->post('username'),
            'password'=>md5($this->input->post('password')),
            'nama'=> $this->input->post('nama'),
            'level'=>$this->input->post('level'),
        );
        $this->model_app->updateData('tbl_pegawai',$data,$id);
        redirect("master/pegawai_data");
    }

    function edit_data_pegawai(){
        $id= $this->uri->segment(3);
        $data=array(
            'title'=>'Data Admin',
            'data'=>$this->model_app->getPegawai($id),
        );
        $this->load->view('element/v_header',$data);
        $this->load->view('pages/admin/v_edit_pegawai');
        $this->load->view('element/v_footer');
    }

//    ========================== DELETE =======================
    function hapus_barang(){
        $id['kd_barang'] = $this->uri->segment(3);
        $this->model_app->deleteData('tbl_barang',$id);
        redirect("master/barang_data");
    }
    function hapus_pelanggan(){
        $id['kd_pelanggan'] = $this->uri->segment(3);
        $this->model_app->deleteData('tbl_pelanggan',$id);
        redirect("master/pelanggan_data");
    }
    function hapus_supplier(){
        $id['kd_supplier'] = $this->uri->segment(3);
        $this->model_app->deleteData('tb_supplier',$id);
        redirect("master/supplier_data");
    }    
    function hapus_pegawai(){
        $id['kd_pegawai'] = $this->uri->segment(3);
        $this->model_app->deleteData('tbl_pegawai',$id);
        redirect("master/pegawai_data");
    }
}